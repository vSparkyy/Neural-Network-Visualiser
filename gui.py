...
class Slider:
    ...
    def __init__(self, position, max_value, min_value=0, extra_text=None):
        ...
        if not self.locked:
            ...
            if buttons[0]:
                if self.slider_rect.collidepoint(mouse_pos):
                    new_value = (mouse_pos[0] - self.x) * (self.max_value - self.min_value) / (BAR_WIDTH - self.slider_width)
                    self.current_value = max(self.min_value, min(new_value, self.max_value))

        surface.blit(BAR_FRAME_IMAGE, (self.x, self.y))
        value_text = f"{str(int(self.current_value)) + self.extra_text}" if self.extra_text else str(int(self.current_value))
        value_font = FONT.render(value_text, False, COLOURS['WHITE'])
        slider_pos = self.x + ((self.current_value - self.min_value) / (self.max_value - self.min_value)) * (BAR_WIDTH - self.slider_width)
        slider_pos = max(self.x, min(slider_pos, self.x + BAR_WIDTH - self.slider_width))
        surface.blit(SLIDER_IMAGE, (slider_pos, self.y))
        surface.blit(value_font, (self.x + 120, self.y - 1))


class TextBox:
    """Class representing a text box."""
    def __init__(self, text, position, font_size=30, colour=COLOURS['WHITE'], target_word=None, target_colour=None):
    ...
    def update(self, surface):
        ...
        for word in words:
            ...
            if word == self.target_word and self.target_colour is not None:
                word_surface = self.font.render(word, False, self.target_colour)
